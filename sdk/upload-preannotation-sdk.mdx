---
title: "Upload Pre-annotations Using SDK"
...
client = LabellerrClient('your_api_key', 'your_api_secret', 'your_client_id')
...
try:
    ...
    result = project.upload_preannotation_by_project_id(
        project_id, 
        client_id, 
        annotation_format, 
        annotation_file
...
try:
    ...
    future = project.upload_preannotation_by_project_id_async(
        project_id, 
        client_id, 
        annotation_format, 
        annotation_file
    ...
    # Check status asynchronously (retry_interval in seconds)
    status_future = project.preannotation_job_status_async(retry_interval=5)
    
    # Retrieve result when ready
    try:
        result = status_future.result(timeout=300)  # 5 minute timeout
...
Select the appropriate method based on your application requirements:

<CardGroup cols={2}>
  <Card title="Synchronous Method" icon="hourglass">
    **Use Cases:**
  ...
  <Card title="Asynchronous Method" icon="rocket">
    **Use Cases:**
    - Web applications
