...
# Step 1: Create an S3 connection (if not already created)
try:
    ...
    print(f"S3 Connection created successfully. Connection ID: {connection_id}")
except LabellerrError as e:
    print(f"S3 connection creation failed: {str(e)}")

# Step 2: Create S3 export
project_id = 'your_project_id'
...
export_config = CreateExportParams(
    ...
    export_folder_path="bucket_name/path/to/folder/"  # Note: trailing slash is important
)
...
try:
    ...
    print(f"S3 export created successfully. Export ID: {export_id}")
    
    # Optionally, check current status
    print(f"Current status: {export._status}")
    
    # Optionally, poll until completion
    final_status = export.status(interval=2.0, timeout=300)
    print(f"Final status: {final_status}")
except LabellerrError as e:
    print(f"S3 export creation failed: {str(e)}")
```
