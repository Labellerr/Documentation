---
title: "Retrieve Projects, Datasets & Templates"
description: "Learn how to retrieve all projects, datasets, and annotation templates associated with a client in Labellerr using the SDK, including linked and unlinked datasets."
icon: database
---
...
You can retrieve all annotation templates associated with a client using the SDK's template listing functionality. This is useful when you want to reuse existing templates instead of creating new ones.

#### Example Usage:

```python Retrieve All Templates lines icon="clipboard-list"
from labellerr.client import LabellerrClient
from labellerr.core.annotation_templates import list_templates
...
try:
    # List all templates for a specific data type
    templates = list_templates(client, DatasetDataType.image)
    
    print(f"Found {len(templates)} templates:")
    for template in templates:
        print(f"- Template ID: {template.template_id}")
        print(f"  Name: {template.template_name}")
        print(f"  Questions: {template.questions}")
        print()
        
except LabellerrError as e:
    print(f"Failed to retrieve templates: {str(e)}")
```

<Tip>
**Filter by Data Type:**
Use `DatasetDataType` enum to filter templates by their supported data type:
- `DatasetDataType.image` - Image annotation templates
- `DatasetDataType.video` - Video annotation templates
- `DatasetDataType.audio` - Audio annotation templates
- `DatasetDataType.document` - Document annotation templates
- `DatasetDataType.text` - Text annotation templates
</Tip>

This method is useful when you need to:

*   List all available templates in your workspace
*   Find a specific template ID to reuse in new projects
*   Review template configurations and questions
*   Avoid creating duplicate templates by checking existing ones first

---

#### Advanced Pagination Examples (Datasets)
...
try:
    ...
    result = project.bulk_assign_files(
        ...
        new_status="review",
        assign_to="ASSIGNEE_EMAIL@company.com"  # Optional: Replace with actual user email
    )
...
result = project.bulk_assign_files(
    ...
    new_status="review",
    assign_to="REVIEWER_EMAIL@company.com"  # Replace with actual reviewer email
)
```
...
<Accordion title="list_templates(client, data_type)" icon="clipboard-list">
...
def list_templates(
    ...
    """
    Retrieves all annotation templates for a specific data type.
    
    Parameters:
        client: LabellerrClient instance
        data_type: DatasetDataType enum value (image, video, audio, document, text)
        
    Returns:
        List of LabellerrAnnotationTemplate objects with properties:
        - template_id: str
        - template_name: str
        - questions: list
        - data_type: str
